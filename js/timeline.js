...
function initTimeline() {
 ...
 tweets = tweets.sort(sortByDateAscending);
  ...
  var nestByDate = d3.nest()
    .key(function(d) { return d3.time.day(d.attributes.created_at); })
    //.entries(tweets);
 //console.log(nestByDate) ;  
  ...
  var date = tweetCrossFilter.dimension(function(d) { return d.attributes.created_at; });
  var dates = date.group(d3.time.day);
  var hour = tweetCrossFilter.dimension(function(d) { return d.attributes.created_at.getHours() + d.attributes.created_at.getMinutes() / 60; });
  var hours = hour.group(Math.floor);

  var dateTag = tweetCrossFilter.dimension(function(d) { return d.attributes.created_at; });
  var datesTags = date.group(d3.time.day);

  var dateTagForChart = tweetCrossFilter.dimension(function(d) { return d.attributes.created_at; });
  var datesTagsForChart = date.group(d3.time.day);

  //define date ranges

  minPossibleDate = moment(d3.min(tweets, function(d) { return d.attributes.created_at; } ) ).startOf("day").toDate();
  maxPossibleDate = moment(d3.max(tweets, function(d) { return d.attributes.created_at; } ) ).endOf("day").toDate();
  ...
  window.updateTimelineFilterByHashtag = function(type) {
    ...
    dateTag = filteredTweetCrossFilter.dimension(function(d) { return d.attributes.created_at; });
    ...
    if(type == "tags") {
      ...
      dateTagForChart = filteredTweetCrossFilterByTag.dimension(function(d) { return d.attributes.created_at; });
  ...
  function tweetList(div) {
    ...
    div.each(function() {
      ...
      date.enter().append("div")
        ...
        .append("div")
          .attr("class", "day")
          .text(function(d) { return formatDate(d.values[0].attributes.created_at); });
      ...
      tweetEnter.append("div")
          .attr("class", "time")
          .text(function(d) { return formatTime(d.attributes.created_at); });
