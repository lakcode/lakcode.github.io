...
function loadDataset() {
	...
	d3.json("data/Slang_retweets_n2.json", function(error, json) {
		...
		data.nodes.forEach(function(node) {
		    node.attributes.create_at = new Date(node.attributes.created_at);
		});
...
function addToHashtagList(link) {
    ...
    link.attributes.hashtags.forEach(function(hashtag) {
	    ...
	    if(tagListID < 0) {
			...
			tag.values.push({
				linkID: link.id,
				linkDate: link.attributes.created_at
			});
